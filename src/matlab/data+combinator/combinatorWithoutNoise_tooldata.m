...
function combinatorWithoutNoise_tooldata(fileDesc, fileEff, fileAff)

fileDesc = 'tool_cont_desc_noheader.txt';
fileEff = 'tool_cont_eff_noheader.txt';
fileAff = 'tool_affData_03_2017.txt';
...
for n = 1:size(eff{1,1},1)
    ...
    indTool   = find(desc{1,1}==eff{1,1}(n));
    indObj = find(desc{1,1}==eff{1,2}(n));
    
    temp_aff(1:(size(indTool,1)*size(indObj,1)),1:27) = NaN;
    
    for Tool = 1:size(indTool,1)
        for Target = 1:size(indObj,1)
            for d = 1:12 % tool descriptor
                temp_aff(Target+(Tool-1)*size(indObj,1),d) = desc{1,d+1}(indTool(Tool));
            end
            for d = 1:12
            ...
            temp_aff(Target+(Tool-1)*size(indObj,1),26) = (eff{1,8}(n) - eff{1,4}(n)); %effectX
            temp_aff(Target+(Tool-1)*size(indObj,1),27) = (eff{1,9}(n) - eff{1,5}(n)); %effectY
