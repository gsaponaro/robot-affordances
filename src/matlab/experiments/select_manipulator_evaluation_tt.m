...
% manipulator selection/ranking with tool->tool network

function quality = select_manipulator_evaluation_tt(action)
    ...
    for o = 4:7
        ...
        for Tool = 1:size(indTool,1)
            for Object = 1:size(indObj,1)
                ...
                % query tt network
                posterior = train_tool_select_manipulator(query, 'pca_2n2c2v_tt');
