...
function [score] = train_hand_test_tool
%%
...
data = load('../data+combinator/affData_03_2017.txt');
data_normalize = zeros(size(data));
[data_normalize(:,1:24),mu_train,sigma_train] = zscore(data(:,1:24));%for new data use (data(:,1:24)-mu)./sigma; only works since 2016b
data_normalize(:,25:end) = data(:,25:end);
train_rows = size(data,1); % 100% of the data
training_data = data_normalize(1:train_rows,:);

%test_data=load('testData.txt');

featuresH = training_data(:,1:12);
featuresO = training_data(:,13:24);
%%pmtk required
[latentH,pcH] = pca(featuresH);
...
SCORE = featuresH*pinv(pcH(:,1:components)');
...
%%
score=[ featuresH*pinv(pcH(:,1:components)') featuresO*pinv(pcO(:,1:components)')];
% score=[ SCORE SCORE2];
...
save('../data+combinator/pca_2n2c2v_ht.mat','BN','ranges','pcH','pcO','components','mu_train','sigma_train');
...
data_test = load('../data+combinator/tool_affData_03_2017.txt'); 
